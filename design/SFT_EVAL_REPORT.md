...
4. 量化复评（INT4 vs bf16 混淆对差 ≤3pp）。
   → ✅ 已执行并**判定弃量化**（见第九节）。

---

## 九、INT4 量化回归结论（L3，2026-09-13）：弃量化，bf16 部署

### 方法

`autoawq 0.2.9` 要求 transformers≤4.x，与本机 transformers 5.8 不兼容；
改用 vLLM 官方 `llmcompressor 0.13`（compressed-tensors W4A16，vLLM 原生加载），
独立环境 `mff_sft_quant`（不污染训练/服务环境）。校准语料=训练集退化版 prompt 128 条。
`tools/mff_quantize.py` 支持 `W4A16`（RTN，data-free）与 `W4A16_GPTQ`（带校准误差校正）。

### 结果（同一 eval 集，关仲裁）

| 指标 | bf16（v3） | INT4 RTN | INT4 GPTQ | 门槛 |
|---|---|---|---|---|
| 根因准确率（故障子集 n=170） | **99.4%** | 0.0% | 58.2% | ≥90% |
| 混淆对准确率（n=60） | **100%** | 0.0% | 55.0% | ≥90% |
| 无故障误报率（n=20） | **0%** | 0% | 95.0% | ≤5% |
| JSON 合法率 | 100% | 100% | 99.0% | ≥99% |
| 证据忠实率 | 100% | 100% | 99.5% | ≥98% |
| 防幻觉通过率 | 100% | 100% | 100% | 100% |
| 时延 P95 | 1.75s | 0.74s | 0.95s | ≤5s |

- **RTN（W4A16）灾难性退化**：全部故障样本输出"无故障（工况切换正常波动）"，故障准确率 0%。
- **GPTQ（W4A16_GPTQ）部分退化**：根因 58.2%（较 bf16 掉 **41.2pp**）、混淆对 55.0%
  （掉 **45.0pp**）、误报率飙到 95%。日志显示多层 Hessian 数值不稳回退 RTN。
- 两者混淆对降幅均**远超 3pp 阈值**。

### 结论

按 `docs/LLM_SFT后训练实施指南.md` 6.5.5 决策树（L3.2 不过 → 换 AWQ 重试一次 →
仍不过 → 弃量化），**判定放弃 INT4，采用 bf16 部署**。

根因分析：v3 模型是**记忆型过拟合**（val loss ~0.0004），权重对量化误差极其敏感；
4bit 分组量化（group 128）的扰动直接破坏了其高度特化的输出映射。
这与 MS5 主结论（"99% 准确率记忆型 SFT"）在机制上一致。

产物 `~/models/mff-sft-minicpm5-2b-int4-gptq`（2.0GB）留档为**负面证据**，不部署。
`tools/mff_quantize.py` 与 `mff_sft_quant` 环境保留，供后续若换底座/更大模型时复用。
